...
func (le *LocationExpression) String() string {
	return fmt.Sprintf("Row: %d, Column: %s, Where: %s", le.RowIndex, le.ColIndex, le.Filter)
}
...
// PrefixExpression struct represents the prefix expression in the program
...
// StringLiteral struct represents the string literal in the program
